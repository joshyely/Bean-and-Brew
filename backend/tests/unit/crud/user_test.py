...
def test_get_user_by_id(temp_db, valid_id:int, valid_email:str, record_data):
    ...
    """
    result = user.get_user_by_id(temp_db, valid_id)
...
def test_get_user_by_email(temp_db, valid_email:str, record_data):
    ...
    """
    result = user.get_user_by_email(temp_db, valid_email)
...
@pytest.mark.parametrize('email,password', [
    pytest.param(lambda request: request.getfixturevalue('valid_email'), 'Wrongpassword£124', id='Correct Email, Incorrect Password'),
    pytest.param('wrongemail@example.com', lambda request: request.getfixturevalue('valid_password'), id='Correct Password, Incorrect Email'),
...
def test_authenticate_invalid(temp_db, email:str, password:str, record_data):
    ...
    record_data(
        expected=None,
        actual=result,
        test_data=[f'memory db session: {temp_db}', f'email: {email}', f'password: {password}']
    )
    assert result == None
